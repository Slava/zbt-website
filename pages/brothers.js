...
import { Row, Splash, Split, ImgGallery } from "./_sharedComponents";
...
export default function (props) {
  ...
  const onSelect = (brotherDoc) =>
    _brother.setState(brotherDoc) + openBrotherFn();
    ...
    <DocumentTitle title={"Brothers | " + config.siteTitle}>
      <div>
        ...
        <div className="contents typography">
          <h1>Brothers</h1>
          <p>
            ZBT is a non-pledging, historically Jewish fraternity open to all
            men of good character. Our house attracts smart and energetic young
            men interested in sciences and engineering with wide range of
            athletic and academic interests. Many brothers are very active on
            campus, leading various student organisations and clubs, dancing in
            troups, trading, playing poker and teaching web development to the
            wider MIT community. Many brothers are also involved in student
            clubs working with kids in Boston area such as{" "}
            <a href="http://campkesem.org/mit">Camp Kesem</a> and{" "}
            <a href="http://mitdynamit.weebly.com/">DynaMIT</a>.
          </p>
        ...
        <BrotherPane
          ref={(ref) => (_brother = ref)}
          cb={(f) => (openBrotherFn = f)}
        />
      </div>
    </DocumentTitle>
  );
  ...
  render() {
      ...
      return (
        <div key={name} className="brother-card">
          <a
            onClick={(e) => {
              e.preventDefault();
              this.props.onSelect(brotherDoc);
            }}
          >
            <img
              src={imgurl}
    ...
    return (
      <div className="class-gallery">
        ...
        <div className="cards">{brothersCards}</div>
      </div>
    );
  }
}
